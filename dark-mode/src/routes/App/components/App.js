  ...
  return (
    <div className={`app ${isDark ? "dark-mode" : ""}`}>
      <div className="level">
        ...
        {!isDark &&
              <button className="app__dark-mode-btn icon level-right" onClick={() => setIsDark(true)}>
                <FontAwesomeIcon icon={faMoon} />
              </button>
        }
        {/* --The button that should toggle light mode-- */}
        {isDark && 
              <button className="app__dark-mode-btn icon sun level-right" onClick={() => setIsDark(false)}>
